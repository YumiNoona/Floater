...
func plunder():
	for item in _booty:
		...
		item.freeze = false
		item.apply_impluse(Vector2.UP * Global.pixelpertile * _rng.randf_range(5,10) + Vector2.RIGHT * Global.pixelpertile * _rng.randf_range(-1,1))
...
func throw_padlock():
	...
	padlock.apply_impluse(Vector2.UP * Global.pixelpertile + Vector2.RIGHT * Global.pixelpertile * _rng.randf_range(-1,1))
	get_parent().add_child(padlock)

func _on_body_entered(body: Node2D):
	if body is Character:
		if _is_locked && Unit.data.has_key:
			_is_locked = false
			$/root/Game.use_key()
		if not _is_locked:
			_is_open = true
